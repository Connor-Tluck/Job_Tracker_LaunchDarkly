...
#### Step 5: Import LaunchDarkly Feature Flags (REQUIRED)

**IMPORTANT:** The application requires **30 feature flags** to be created in your LaunchDarkly project before it will function correctly. This step is mandatory - the application will not work properly without these flags.

You have two options to install the flags:
...
1. **OpenAI API Key Required:** The chatbot requires your OpenAI API key for actual API calls. Without this key, the chatbot will not function.

2. **LaunchDarkly AI Config Setup Required:** The chatbot uses LaunchDarkly AI Configs for dynamic prompt and model management. **You must configure AI Configs in your LaunchDarkly project instance** for the chatbot to work properly. Without AI Configs set up in LaunchDarkly, the chatbot may not function as expected or may return errors. To set up AI Configs:
   - Navigate to your LaunchDarkly project → Feature Flags
   - Find the flag that controls the chatbot (or create one)
   - Configure AI Configs in the flag settings to define prompts and model behavior
   - See the Assignment Docs page (`/admin/assignment-satisfaction`) or Examples page (`/admin/examples`) for detailed setup instructions
...
- **AI Config Setup:** Ensure LaunchDarkly AI Configs are configured in your LaunchDarkly project instance. The chatbot requires AI Configs to be set up in LaunchDarkly to function properly. Without proper AI Config setup, the chatbot may return errors or not work as expected.
